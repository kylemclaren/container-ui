...
| **Machines** | Persistent Linux environments (`container machine`): list with live CPU/memory, inspect (image, platform, disk, home mount, mapped user), **create & boot** with streamed progress, start/stop, **one-click shell** in your terminal (boots the machine if needed), logs, set default, edit CPUs/memory/home-mount, delete. |
